...
> основа для технического задания и оценки. Открытые вопросы проработаны; принятые рабочие
> решения помечены значком ✅ и подтверждаются на старте работ.
>
...
> Это одна роль; в документе — **Менеджер (ADM-M)**. По требованиям проекта ролей две;
> архитектура допускает добавление ролей (финансы, поддержка и др.) без переделки.

### 0.1 Статусная модель заказа (сквозное требование) — ✅ решение принято

**Принятое решение: у заказа ОДНО поле статуса.** Разные стороны устанавливают
...
✅ **Решено: корзина нужна** — см. PUB-G-05; кнопка деталки добавляет напиток в корзину, оплата — из корзины.
...
### PUB-G-05 · Корзина из нескольких напитков ✅ добавлено по итогам проработки
...
✅ **Решено: realtime — WebSocket** (меньше нагрузки на сервер и задержек, чем поллинг).
...
✅ **Решено:** шкала 👍/👎 · лимит — 1 купон на заказ · порог 15 минут захардкожен.
...
✅ **Решено: списываемый напиток выбирает клиент.**
...
### PUB-A-09 · Язык пользователя (локализация на публичном сайте) ✅ добавлено по итогам проработки
...
2. **Тип выбора** добавок в категории (✅ уточнение к исходным требованиям): **один** (например, один вид молока) / **несколько** (несколько разных видов) / **счётчик** (количество порций одной добавки).
...
2. ✅ Уточнено: привязка единицы к добавке происходит **на экране деталки/редактирования добавки** (ADM-S-03) — выбором из списка единиц, доступных в админке; отдельный экран справочника нужен только для пополнения списка.
...
✅ Функционал проработан детально: блок разбит на user stories — клиентская часть
...
> 🧩 **Опциональный модуль** — оценивается отдельно и может быть исключён из объёма работ
> без влияния на остальной функционал (базовая модель статусов §0.1 работает и без ветки `refund`).
...
| ADM-S-12 | Купоны (реестр) | Супер-админ | — | **новое** |
...
### 6.2 ❓ Открытые вопросы (требуют решения бизнеса)

| # | Вопрос | Блокирует |
|---|---|---|
| Q2 | **SMS-подтверждение телефона**: на старте вход без кода (телефон — контакт для выдачи, OTP-инфраструктура готова и включается флагом); выбрать провайдера SMS и момент включения | PUB-G-04 |
...
| Q16 | **Вход в админку**: реализовано рабочее решение email+пароль; подтвердить, нужна ли 2FA для супер-админа | ADM-S-06 |
...
1. Согласовать оставшиеся вопросы §6.2 (приоритет: Q2, Q10, Q14 — влияют на модель данных и оценку).
