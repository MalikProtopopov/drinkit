...
const STEP_SHORT: Record<string, string> = {
  new: "Новый", in_progress: "В работе", ready: "Готов", completed: "Передан",
};
...
const fmtDateTime = (s: string) =>
  parseTs(s).toLocaleString("ru-RU", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" });

function eventLabel(type: string, status?: string | null): string {
  switch (type) {
    case "created": return "Заказ создан";
    case "paid": return "Оплата прошла";
    case "coupon_applied": return "Применён купон";
    case "rated": return "Оценка клиента";
    case "refund": return "Оформлен возврат";
    case "arrived": return "Клиент прибыл к точке";
...
function WaitBadge({ minutes }: { minutes: number }) {
  const c = minutes >= 15
    ...
    : minutes >= 5
      ? { bg: "#FDF3DB", fg: "#B45309" }
      : { bg: "#DDEDE0", fg: "#166534" };
  const label = minutes < 1 ? "только что" : minutes < 60 ? `ждёт ${minutes} мин`
    : `ждёт ${Math.floor(minutes / 60)} ч ${minutes % 60} мин`;
  return (
...
function Detail({ id }: { id: number }) {
  ...
  const unit = (u: string) => (u === "ml" ? " мл" : u === "pcs" ? " шт" : " г");

  const nextAction =
    order.status === "new" ? { label: "Взять в работу →", run: () => adminApi.take(order.id), ok: "Взят в работу" }
    : order.status === "in_progress" ? { label: "Готово, ожидает выдачи →", run: () => adminApi.setStatus(order.id, "ready"), ok: "Готов — ждём клиента" }
    : order.status === "ready" ? { label: "Передан клиенту ✓", run: () => adminApi.setStatus(order.id, "completed"), ok: "Передан клиенту" }
    : null;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
        <div style={{ fontSize: 22, fontWeight: 800, letterSpacing: "-0.02em" }}>Заказ #{order.number}</div>
        {order.createdAt && <span className="admin-meta">оформлен {fmtDateTime(order.createdAt)}</span>}
      ...
      {order.arrived && isActive && (
        <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 18px", borderRadius: 14,
          ...
          <div>
            Клиент уже на месте — машина <strong>{order.emirate} {order.carPlate}</strong>.
            {order.status !== "ready" && <span style={{ color: "#B45309" }}> Заказ ещё готовится — приоритет!</span>}
          </div>
        </div>
      )}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 16, alignItems: "start" }}>
        ...
        <div className="admin-panel">
          <div className="admin-panel-head">
            <div className="admin-panel-title">Что готовить · {order.items.length} поз. / {drinksTotal} шт</div>
          </div>
            ...
            <div key={it.id} style={{ display: "flex", gap: 14, padding: "16px 20px",
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  ...
                  {it.paidByCoupon && <span className="admin-pill accent">по купону</span>}
                </div>
                {it.customName && (
                  <div className="admin-meta" style={{ marginTop: 2 }}>
                    Имя от клиента · базовый напиток: <strong>{it.drinkNameEn}</strong>
                  </div>
                )}

                <div style={{ marginTop: 10 }}>
                  {it.addons.length === 0 ? (
                    <span className="admin-meta">Без добавок — базовый рецепт</span>
                  ) : (
                    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                      <div className="admin-meta" style={{ fontWeight: 700, marginBottom: 2 }}>Добавки:</div>
                      {it.addons.map((a, i) => (
                        <div key={i} style={{ display: "flex", justifyContent: "space-between", gap: 10, fontSize: 13.5 }}>
                          ...
                          <span className="admin-meta" style={{ whiteSpace: "nowrap" }}>
                            {a.price > 0 ? `+${(a.price * a.portions).toFixed(0)} AED` : "включено"}
                          </span>
                        </div>
                      ))}
                    </div>
          ...
          <details style={{ borderTop: "1px solid var(--a-rule)" }}>
            <summary style={{ cursor: "pointer", padding: "12px 20px", fontWeight: 600, fontSize: 13, color: "var(--a-ink-soft)" }}>
              Снимок заказа (оригинал, как видел клиент) — для проверки
            </summary>
            <div style={{ padding: "0 20px 16px" }}>
              <div className="admin-meta" style={{ marginBottom: 8 }}>
                Зафиксировано на момент оплаты в локали клиента. Рабочие названия выше — актуальные (EN).
              </div>
              {order.items.map((it) => (
                <div key={it.id} style={{ padding: "8px 0", borderTop: "1px dashed var(--a-rule)", fontSize: 13 }}>
                  <div>
                    ...
                    {it.customName && <span className="admin-meta"> · базовый: {it.drinkName}</span>}
                  </div>
                  {it.addons.length > 0 && (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div className="admin-panel">
            <div className="admin-panel-head"><div className="admin-panel-title">Обработка заказа</div></div>
          ...
          {/* клиент и выдача */}
          <div className="admin-panel">
            <div className="admin-panel-head"><div className="admin-panel-title">Клиент и выдача</div></div>
            <div className="admin-panel-body">
              ...
              <div className="admin-field"><label className="admin-label">Телефон</label>
                <a href={`tel:${order.phone}`} className="admin-mono"
                  ...
                  {order.phone} <span style={{ fontSize: 12 }}>· позвонить</span>
                </a></div>
              {order.rating && (
            ...
            <div className="admin-panel-body" style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                ...
                <span className="admin-pill" style={order.paymentStatus === "paid"
                  ...
                  {order.paymentStatus === "paid" ? "оплачен" : order.paymentStatus}
                </span>
              </div>
          ...
          <div className="admin-panel">
            <div className="admin-panel-head"><div className="admin-panel-title">История статусов</div></div>
            <div className="admin-panel-body">
                      ...
                      <div className="admin-timeline-meta">
                        ...
                        {h.byStaffId ? (
                          <Link href={`/admin/staff/${h.byStaffId}`}
                            ...
                            {h.byStaffName ?? `сотрудник #${h.byStaffId}`} →
                          </Link>
                        ) : h.byUserId ? "клиент" : "система"}
                      </div>
                    </div>
                  </div>
                ))}
      ...
      <ConfirmDialog
        ...
        message="Статус станет «возврат», платёж пометится refunded. Клиент увидит возврат на странице заказа."
