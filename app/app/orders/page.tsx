...
export default function OrdersPage() {
  ...
  return (
    <div className="flex-1 flex flex-col">
      <TopBar title={t("Orders", "الطلبات")} showCart back="/home" />

      <div className="flex-1 overflow-y-auto px-4 pb-4">
        {orders === null ? (
          <div className="text-center muted py-20">{t("Loading…", "جارٍ التحميل…")}</div>
        ) : orders.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center py-20 text-center">
            <div className="mb-4">
              <DrinkArt glass="paper" liquid="#C2A07A" foam="#F5E8D0" size={140} />
            </div>
            <div className="text-h3 mb-1">{t("Your orders will appear here", "ستظهر طلباتك هنا")}</div>
            <div className="muted text-body mb-4">
              {guest ? t("Sign in to see your history", "سجّل الدخول لرؤية سجلّك") : t("Place your first one — it's quick", "اطلب أول طلب — الأمر سريع")}
            </div>
            {guest && (
              <Link href="/auth/phone" className="btn-pill btn-primary px-8">{t("Sign in", "تسجيل الدخول")}</Link>
            )}
          </div>
        ) : (
